...
def main():
    ...
    input_csv = 'C:\\Users\\Eleanor\\Dropbox\\Honors Research\\Tweets\\TwitterAPIWrapper\\Candidates2020_tweets_3accts_all.csv'
    # input_csv = 'sample_tweets.csv'
    output_csv = 'sample_many_cats.csv'

    topics = t.get_topics()

    start = time.time()
    print('reading')
    tweets_df = pd.read_csv(input_csv).sample(1000)
    print('cleaning')
    ...
    #                 counter[topic] += 1
    #         max_topic = max(counter, key=lambda val: counter[val])
    #         if counter[max_topic] == 0:
    #             continue
    #         for topic in topics.keys():
    #             if topics[topic] == topics[max_topic] and topic != max_topic:
    #                 max_topic += ',' + topic
    #         tweets_df.loc[index, ['category']] = max_topic

    # x = tweets_df[['tweet_text',  'category']]
