...
def get_candidates_profiles(url):
    ...
    # full link is not available in href, only extension which is why I need to prepend the rest here
    return [(a_tag.text, 'https://ballotpedia.org' + a_tag['href']) for a_tag in a_tags]
...
def main():
    ...
    output_filename = 'Candidates2018_set_{0}.csv'.format(datetime.now().strftime("%H%M%S_%m%d%Y"))
    havent_written = True
    ...
    for i in range(start_at, start_at+num_to_do+1):
        ...
        try:
            ...
            print('{0}: {1}'.format(i, profile_info))
            consec_exceptions_count = 0
        except Exception as e:
            consec_exceptions_count += 1
            print('{0}: exception'.format(i), names[i], consec_exceptions_count)
            print(e)
            if consec_exceptions_count > 10:
                break

        if i % 30 == 0 or i == len(names) - 1:
