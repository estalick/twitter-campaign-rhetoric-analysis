...
# IMPORT CATEGORIZED TWEET DATA AND CREATE TIME-SEPARATED CSVs 
Feb 25 2020, CDC declares that covid is headed towards Pandemic status
May 25 2020, Police officers kill George Floyd
(these three files will be individually grouped, then timestamped and combined)
```{r}
library(lubridate)
...
tweets_data <- read.csv("categorized_tweets_2020.csv")

tweets_data$tweet_timestamp <- mdy_hms(tweets_data$tweet_timestamp)

# pre_covid and pre_blm vars
tweets_data$pre_covid[tweets_data$tweet_timestamp<as.Date("2020-02-25")] <- TRUE
tweets_data$pre_covid[tweets_data$tweet_timestamp>=as.Date("2020-02-25")] <- FALSE
tweets_data$pre_blm[tweets_data$tweet_timestamp<as.Date("2020-05-25")] <- TRUE
tweets_data$pre_blm[tweets_data$tweet_timestamp>=as.Date("2020-05-25")] <- FALSE

# sort into 3 groups
precov_data <- subset(tweets_data, subset = pre_covid==TRUE)
postcov_preblm_data <- subset(tweets_data, subset = (pre_covid==FALSE & pre_blm==TRUE))
postblm_data <- subset(tweets_data, subset = pre_blm==FALSE)

write.csv(precov_data,"categorized_tweets_2020_precov.csv", row.names = TRUE)
write.csv(postcov_preblm_data,"categorized_tweets_2020_postcov_preblm.csv", row.names = TRUE)
write.csv(postblm_data,"categorized_tweets_2020_postblm.csv", row.names = TRUE)
```
...
### data importation
setwd("C:/Users/Zach/Desktop/Eleanor_2021/Penn_Senior_Thesis/tweet_categorization/output_csvs")
data <- read.csv("grouped_tweets_2020_timestamped.csv")

### IV and CV creation (gender, party, incumbency status, postcov, postblm)
...
temp_candidates_data <- read.csv("candidates_2020.csv")
data$incumbent <- temp_candidates_data$incumbent[match(data$candidate_id, temp_candidates_data$candidate_id)]

data$postcov[data$post_covid==FALSE] <- 0
data$postcov[data$post_covid==TRUE] <- 1

data$postblm[data$post_blm==FALSE] <- 0
data$postblm[data$post_blm==TRUE] <- 1
...
myvars <- c("male","republican", "incumbent", "postcov", "postblm",
...
stargazer(mydata, type="text", title = "2020 Descriptive Stats Table 2: Twitter Discussion",
          covariate.labels = c("Male","Republican","Incumbent", "Post-Pandemic", "Post-BLM",
                               "% Feminine Tweets", "% Masculine Tweets", "% COVID-19 Tweets",
...
fem_multivar <- lm(feminine_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
fem_multivar_robust <- coeftest(fem_multivar, vcov = vcovHC(fem_multivar, type="HC1"))

stargazer(fem_bivar_robust,fem_multivar_robust, title="2020 OLS Regression 1", align=TRUE, type="text", 
          ...
          model.names=FALSE, order = c("Constant", "Male", "Republican","Incumbent","Post-Pandemic","Post-BLM"),
          covariate.labels=c("Constant","Male","Republican","Incumbent", "Post-Pandemic","Post-BLM", "Male:Post-Pandemic", "Male:Post-BLM", "Republican:Post-Pandemic", "Republican:Post-BLM"))
...
masc_multivar <- lm(masculine_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
masc_multivar_robust <- coeftest(masc_multivar, vcov = vcovHC(masc_multivar, type="HC1"))

stargazer(masc_bivar_robust,masc_multivar_robust, title="2020 OLS Regression 2", align=TRUE, type="text", 
...
WR_multivar <- lm(womens_rights_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
WR_multivar_robust <- coeftest(WR_multivar, vcov = vcovHC(WR_multivar, type="HC1"))

stargazer(WR_bivar_robust,WR_multivar_robust, title="2020 OLS Regression 1a", align=TRUE, type="text", 
...
FC_multivar <- lm(familycare_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
FC_multivar_robust <- coeftest(FC_multivar, vcov = vcovHC(FC_multivar, type="HC1"))

stargazer(FC_bivar_robust,FC_multivar_robust, title="2020 OLS Regression 1b", align=TRUE, type="text", 
...
ED_multivar <- lm(education_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
ED_multivar_robust <- coeftest(ED_multivar, vcov = vcovHC(ED_multivar, type="HC1"))

stargazer(ED_bivar_robust,ED_multivar_robust, title="2020 OLS Regression 1c", align=TRUE, type="text", 
...
HC_multivar <- lm(healthcare_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
HC_multivar_robust <- coeftest(HC_multivar, vcov = vcovHC(HC_multivar, type="HC1"))

stargazer(HC_bivar_robust,HC_multivar_robust, title="2020 OLS Regression 1d", align=TRUE, type="text", 
...
MT_multivar <- lm(military_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
MT_multivar_robust <- coeftest(MT_multivar, vcov = vcovHC(MT_multivar, type="HC1"))

stargazer(MT_bivar_robust,MT_multivar_robust, title="2020 OLS Regression 2a", align=TRUE, type="text", 
...
EN_multivar <- lm(economics_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
EN_multivar_robust <- coeftest(EN_multivar, vcov = vcovHC(EN_multivar, type="HC1"))

stargazer(EN_bivar_robust,EN_multivar_robust, title="2020 OLS Regression 2b", align=TRUE, type="text", 
...
FA_multivar <- lm(foreign_affairs_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
FA_multivar_robust <- coeftest(FA_multivar, vcov = vcovHC(FA_multivar, type="HC1"))

stargazer(FA_bivar_robust,FA_multivar_robust, title="2020 OLS Regression 2c", align=TRUE, type="text", 
...
PO_multivar <- lm(policing_ratio ~ male + republican + incumbent + postcov + postblm + (male*postcov) + (male*postblm) + (republican*postcov) + (republican*postblm), data = mydata_omitNAs)
PO_multivar_robust <- coeftest(PO_multivar, vcov = vcovHC(PO_multivar, type="HC1"))

stargazer(PO_bivar_robust,PO_multivar_robust, title="2020 OLS Regression 2d", align=TRUE, type="text", 
